...
private:
    ...
    // Implementación simple de hash (usando hash estándar de C++)
    string generarHashSimple(const string& texto) {
        hash<string> hasher;
        size_t hashValue = hasher(texto);
        
        stringstream ss;
        ss << hex << hashValue;
        
        // Simular formato SHA-256 (64 caracteres hex)
        string resultado = ss.str();
        while (resultado.length() < 64) {
            resultado = "0" + resultado;
        }
        if (resultado.length() > 64) {
            resultado = resultado.substr(0, 64);
        }
...
int main() {
    try {
        ...
        cout << "Versión Compatible con Dev C++" << endl;
        cout << "=============================================" << endl;
        ...
        cout << "- Algoritmo de encriptación optimizado" << endl;
